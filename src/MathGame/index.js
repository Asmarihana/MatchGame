...
class MathGame extends Component {
  ...
  componentDidMount() {
    this.timerID = setInterval(this.timeInterval, 1000)
  }
  ...
  render() {
    const {tab, imgUrl, time, score, isGameOver} = this.state
    const thumbnailsList = imagesList.filter(each => each.category === tab)
    ...
    return (
      <div className="background">
        ...
        <div>
          {isGameOver ? (
            <GameOver score={score} playGameAgain={this.playGameAgain} />
          ) : (
